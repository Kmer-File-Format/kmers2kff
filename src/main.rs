...
fn main() -> Result<()> {
	...
	for kmer in bucket.keys() {
		...
		counts.extend(&bucket.get(&current).ok_or(anyhow!("counts conversion"))?.to_le_bytes());
	    }
		...
		fusion.push(nuc);
		seens.insert(current);
		counts.push(*bucket.get(&current).ok_or(anyhow!("counts conversion"))?);
	    }

	    let mini_pos = String::from_utf8(fusion.clone())?.find(&seq2bits::kmer2seq(b_id, params.m)).unwrap() as u64;
	...
	writer.write_minimizer_seq_section(&seq2bits::kmer2seq(b_id, params.m).into_bytes(), &mini_poss[..], &sequences[..], &datas)?;
    }
    
    Ok(())
}

fn predecessor(kmer: u128, k: u8, set: &std::collections::HashMap<u128, u8>, seens: &mut std::collections::HashSet<u128>) -> Option<(u128, u8)> {
    let sub = kmer >> 2;
...
fn successor(kmer: u128, k: u8, set: &std::collections::HashMap<u128, u8>, seens: &mut std::collections::HashSet<u128>) -> Option<(u128, u8)> {
...
fn create_bucket(input: &String, prefix: &String, k: u8, m: u8) -> Result<std::collections::HashSet<u128>> {
    let mut bob = std::collections::HashSet::<u128>::new();
    
    let input = std::io::BufReader::new(std::fs::File::open(input).with_context(|| "Open input file")?);
    let mut reader = csv::ReaderBuilder::new().delimiter(b',').has_headers(false).from_reader(input);
	...
	let mut bucket = if bob.contains(&minimizer) {
	     std::fs::OpenOptions::new().append(true).create(true).open(format!("{}{}", prefix, minimizer)).with_context(|| "Open bucket file")?
	} else {
	    bob.insert(minimizer);

	    std::fs::File::create(format!("{}{}", prefix, minimizer)).with_context(|| "Open bucket file")?
	};
	
	write!(bucket, "{},{}\n", kmer, &record[1]).with_context(|| "Write kmer")?;
    }
